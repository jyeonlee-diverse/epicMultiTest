Shader "Custom/GlitterGlassBead"
{
	Properties
	{
		...
		_GlitterScale("Glitter Scale (Lower => larger blobs)", Range(0.5,120)) = 40
		...
		_GlitterDriftAmount("Glitter Drift Amount", Range(0,0.5)) = 0.15
		_GlitterDriftSpeed("Glitter Drift Speed", Range(0,5)) = 0.8
	...
	SubShader
	{
		...
		Pass
		{
			...
			float _GlitterDriftAmount;
			float _GlitterDriftSpeed;
			float4 _ColorA;
			float4 _ColorB;
			float4 _ColorC;
			float _ColorBlend;
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float3 drift = (sin(p * 1.7 + _Time.y * _GlitterDriftSpeed) + cos(p * 2.1 + _Time.y * (_GlitterDriftSpeed * 0.9))) * (_GlitterDriftAmount * 0.5);
				p += drift;

				float n = noise3(p);
				float n2 = noise3(p * 2.7 + 4.1);
				float sparkle = smoothstep(1.0 - _GlitterDensity, 1.0, n);
				sparkle *= smoothstep(1.0 - _GlitterDensity, 1.0, n2);
				float sizeT = saturate((_GlitterSize - 0.02) / 0.38);
				float sharpness = lerp(14.0, 3.0, sizeT);
				sparkle = pow(sparkle, sharpness);

				float phase = noise3(p + 13.71);
				float blink = 0.5 + 0.5 * sin(_Time.y * _GlitterSpeed + phase * 6.2831853);
				float viewSparkle = 1.0 + _GlitterViewStrength * pow(1.0 - viewDot, _GlitterViewPower);

				float mixAB = frac(noise3(p + 2.11) + 0.35 * sin(_Time.y * 0.7 + phase));
				float3 colAB = lerp(_ColorA.rgb, _ColorB.rgb, mixAB);
				float mixC = step(0.5, _ColorBlend) * noise3(p + 7.31);
				float3 glitterColor = lerp(colAB, _ColorC.rgb, mixC);

				float glitterMask = sparkle * blink * viewSparkle * sphereMask;
