	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float3 baseP = i.objPos * _GlitterScale;
				float3 baseCell = floor(baseP);
				float3 driftDir = normalize(hash33(baseCell) - 0.5);
				float driftPhase = hash31(baseCell + 4.37) * 6.2831853;
				float3 drift = driftDir * (sin(_Time.y * _GlitterDriftSpeed + driftPhase) * _GlitterDriftAmount);

				float3 p = baseP + drift;
				float3 cell = baseCell;
